...
```bash
sample-sam-stepfunctions$ sam build --use-container
```
...
```bash
sample-sam-stepfunctions$ sam logs -n StockCheckerFunction --stack-name sample-sam-stepfunctions --tail
```
...
```bash
sample-sam-stepfunctions$ pip install pytest pytest-mock --user
sample-sam-stepfunctions$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name sample-sam-stepfunctions
```
